        ...
        if(!isset($_SESSION['user_info'])){
            if (isset($_REQUEST['login_submit'])) {
                ...
                if (count($user_info) > 0) {
                    ...
                    if( $_SESSION['user_info'][0]["group_id"]==2){
                    header('Location:user_controller.php?action=list');
                    }else if($_SESSION['user_info'][0]["group_id"]==1){
                    header('Location:../../manager/controllers/plan_controller.php?action=list');
                    }else if($_SESSION['user_info'][0]["group_id"]==0){
                    header('Location:../../administrators/controllers/user_controller.php?action=list');
                    }
                } else {
                    //login fail
...
//        unset($_SESSION['user_info']);
    ...
    function listAction(){
        ...
        if(isset($_SESSION['user_info'])){
            ...
            if(isset($_REQUEST['search_data'])){
                ...
                $data = $this->user_model->getAllUsers($data);
            }
    ...
    function editAction($data){
            ...
            if ($result) {
                ...
                header('Location:user_controller.php?action=editPage&id=' . $data['MaKH']);
            }
