...
<!-- start slide -->
	<div class="reset" style="width:100%;" xmlns="http://www.w3.org/1999/html">
			...
			<div class="col-md-9 col-sm-12">
				<div class="row">
					<?php foreach($_SESSION['data'] as $key => $value)
						echo '<span>Name: ' . $value['name'] . '</span> <span>Price: '.$value['price'].'</span></br>';
					?>
				</div>
			</div>
			<!--<div class="col-md-9 col-sm-12">
				<div class="row">
					<form action="<?php /*echo URL_FRONT_END_PART*/?>/controllers/product_controller.php" method="post">
						<input type="hidden" name ="action" value="add_card"/>
						<input type="hidden" name ="id" value="<?php /*echo $_SESSION['data'][0]['id']*/?>"/>
						<button type="submit">Add to card</button>
					</form>
				</div>
			</div>-->
			<!-- button payment -->
			<div><button id="btn_payment">Payment</button></div>
			<!-- button payment -->
			<div id="frm_payment" class="col-md-9 col-sm-12" style="display: none;">
				<form action="<?php echo URL_FRONT_END_PART?>/controllers/bill_controller.php" method="post">
					<input name="action" type="hidden" value="payment"/>
					<input name="custom_name" placeholder="Name">
					<input name="custom_email" placeholder="Email">
					<input name="custom_phone" placeholder="Phone Number">
					<button type="submit">Complete</button>
					<a href="<?php echo URL_FRONT_END_PART?>">Cancel</a>
				</form>
			</div>
			<!-- end right -->
		</div>
